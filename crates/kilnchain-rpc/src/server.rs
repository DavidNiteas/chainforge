...
use axum::Router;
use kilnchain_evm::{Address, EvmExecutor, InMemoryEvmState, U256};
use kilnchain_storage::{InMemoryStorage, StorageEngine};
use serde_json::json;
...
pub struct RpcState {
    pub chain_id: u64,
    pub mempool: RwLock<kilnchain_mempool::Mempool>,
...
impl RpcState {
    pub fn new(chain_id: u64) -> Self {
        RpcState {
            chain_id,
            mempool: RwLock::new(kilnchain_mempool::Mempool::new()),
...
async fn handle_send_raw_transaction(
    ...
    match kilnchain_core::tx::Transaction::decode_rlp(&tx_bytes) {
        Ok(tx) => {
...
async fn handle_get_block_by_number(
    ...
    match state.storage.get(key.as_bytes()).await {
        Ok(Some(data)) => match kilnchain_core::block::Block::decode_rlp(&data) {
...
mod tests {
    ...
    async fn test_eth_send_raw_transaction() {
        let app = app();
        // 构造一笔简单的 RLP 编码交易
        let tx = kilnchain_core::tx::Transaction {
            nonce: 0,
            gas_price: 10,
            gas_limit: 21000,
            to: Some([1u8; 20]),
