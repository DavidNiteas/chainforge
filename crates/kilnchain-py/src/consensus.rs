//! PyO3 bindings for kilnchain-consensus (HotStuff consensus engine).

use kilnchain_consensus::{
    ...
    fn new(block_hash: &[u8], view_number: u64, phase: PyPhase) -> PyResult<Self> {
        ...
        Ok(PyVote {
            inner: Vote {
                ...
                view_number,
                phase: phase.into(),
                voter: kilnchain_crypto::ecdsa::PublicKey::from_bytes(&[0u8; 33]).unwrap(),
                signature: kilnchain_crypto::ecdsa::Signature::from_bytes(&[0u8; 64], 0).unwrap(),
            },
        })
    }
...
impl PyConsensusEngine {
    ...
    fn propose_block<'py>(
        ...
        let rust_txs: Vec<kilnchain_core::tx::Transaction> =
