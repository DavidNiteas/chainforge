...
impl SecretKey {
    ...
    /// 从 32 字节数组构造私钥
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, KilnchainError> {
        if bytes.len() != 32 {
            return Err(KilnchainError::Crypto(format!(
    ...
    pub fn sign(&self, msg: &[u8]) -> Result<Signature, KilnchainError> {
        let secp_sk = SecpSecretKey::from_slice(&self.0)
            .map_err(|e| KilnchainError::Crypto(e.to_string()))?;
...
impl PublicKey {
    ...
    pub fn verify(&self, msg: &[u8], sig: &Signature) -> Result<bool, KilnchainError> {
    ...
    /// 从签名恢复公钥
    pub fn recover_from_msg(msg: &[u8], sig: &Signature) -> Result<Self, KilnchainError> {
        let msg_hash = crate::hash::keccak256(msg);
        let message = Message::from_digest_slice(&msg_hash)
            .map_err(|e| KilnchainError::Crypto(e.to_string()))?;
        // 尝试 recovery id 0..3
        let recid = match sig.recovery_id {
            ...
            2 => RecoveryId::Two,
            3 => RecoveryId::Three,
            _ => return Err(KilnchainError::Crypto("invalid recovery id".to_string())),
        };
...
impl Signature {
    ...
    pub fn from_bytes(bytes: &[u8], recovery_id: u8) -> Result<Self, KilnchainError> {
        if bytes.len() != 64 {
            return Err(KilnchainError::Crypto(format!(
        ...
        if recovery_id > 3 {
            return Err(KilnchainError::Crypto(
...
mod tests {
    ...
    fn test_invalid_secret_key_length() {
        ...
        assert!(matches!(result, Err(KilnchainError::Crypto(_))));
    }
