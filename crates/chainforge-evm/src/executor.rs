...
impl EvmExecutor<InMemoryEvmState> {
    ...
    pub fn transfer(
        ...
        to: Address,
        value: U256,
    ) -> Result<ExecutionResult, ChainforgeError> {
        ...
        let result = evm.transact().map_err(|e| {
            ChainforgeError::InvalidParameter(format!("EVM execution failed: {:?}", e))
        })?;
    ...
    pub fn deploy(
        ...
        let result = evm.transact().map_err(|e| {
            ChainforgeError::InvalidParameter(format!("EVM deploy failed: {:?}", e))
        })?;
