...
impl LightClient {
    ...
    pub fn add_header(&mut self, header: BlockHeader) -> Result<(), KilnchainError> {
    ...
    pub fn verify_transaction(
        ...
        tx_hash: &[u8; 32],
        proof: &MerkleProof,
    ) -> Result<bool, KilnchainError> {
        let header = self
            .get_header_by_number(block_number)
            .ok_or_else(|| KilnchainError::InvalidParameter("block not found".to_string()))?;
