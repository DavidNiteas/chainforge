...
use kilnchain_crypto::ecdsa::{PublicKey, SecretKey, Signature};
...
impl Transaction {
    ...
    /// RLP 解码交易
    pub fn decode_rlp(data: &[u8]) -> Result<Self, KilnchainError> {
        let mut dec = RlpDecoder::new(data);
        let items: Vec<Vec<u8>> = dec.decode_list(|d| Ok(d.decode_bytes()?.to_vec()))?;
        if items.len() != 9 {
            return Err(KilnchainError::Serialization(format!(
        ...
        let to = if items[3].is_empty() {
            ...
            if items[3].len() != 20 {
                return Err(KilnchainError::InvalidParameter(
    ...
    /// 使用私钥对交易进行签名
    pub fn sign(&mut self, sk: &SecretKey) -> Result<(), KilnchainError> {
