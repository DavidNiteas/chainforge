...
mod tests {
    ...
    fn test_crypto_error_display() {
        let err = KilnchainError::Crypto("bad signature".to_string());
        assert_eq!(
            err.to_string(),
    ...
    fn test_serialization_error_display() {
        let err = KilnchainError::Serialization("unexpected EOF".to_string());
        assert_eq!(err.to_string(), "serialization error: unexpected EOF");
    }

    #[test]
    fn test_storage_error_display() {
        let err = KilnchainError::Storage("disk full".to_string());
        assert_eq!(err.to_string(), "storage engine error: disk full");
    }

    #[test]
    fn test_invalid_parameter_display() {
        let err = KilnchainError::InvalidParameter("negative amount".to_string());
    ...
    fn test_error_clone_and_equality() {
        let err1 = KilnchainError::Crypto("x".to_string());
        let err2 = err1.clone();
        assert_eq!(err1, err2);
    }
}
