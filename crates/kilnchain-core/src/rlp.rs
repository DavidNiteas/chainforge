...
impl<'a> RlpDecoder<'a> {
    ...
    pub fn decode_bytes(&mut self) -> Result<&'a [u8], KilnchainError> {
        ...
        if prefix < 0x80 {
            ...
            Err(KilnchainError::Serialization(
                "unexpected list prefix in byte decoding".to_string(),
            ))
        }
    }

    pub fn decode_list<F, T>(&mut self, f: F) -> Result<Vec<T>, KilnchainError>
    where
        F: Fn(&mut RlpDecoder<'a>) -> Result<T, KilnchainError>,
    {
        let prefix = self.read_byte()?;
        if prefix < 0xc0 {
            return Err(KilnchainError::Serialization(
    ...
    pub fn decode_u64(&mut self) -> Result<u64, KilnchainError> {
    ...
    pub fn decode_u128(&mut self) -> Result<u128, KilnchainError> {
        let bytes = self.decode_bytes()?;
        if bytes.is_empty() {
            Ok(0)
        } else {
    ...
    pub fn decode_raw_item(&mut self) -> Result<Vec<u8>, KilnchainError> {
        let start = self.pos;
        let prefix = self.read_byte()?;
    ...
    fn read_byte(&mut self) -> Result<u8, KilnchainError> {
    ...
    fn ensure(&self, len: usize) -> Result<(), KilnchainError> {
