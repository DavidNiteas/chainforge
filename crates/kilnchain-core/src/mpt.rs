...
/// 从 RLP 编码解析 MPT 节点。
fn decode_node(data: &[u8]) -> Result<MptNode, KilnchainError> {
    ...
    match items.len() {
        ...
        n => Err(KilnchainError::Serialization(format!(
...
pub fn verify_proof(
    ...
    key: &[u8],
    proof_nodes: &[Vec<u8>],
) -> Result<Option<Vec<u8>>, KilnchainError> {
...
impl MptProof {
    /// 验证证明并返回值。
    pub fn verify(&self, root: &[u8; 32]) -> Result<Option<Vec<u8>>, KilnchainError> {
