...
#[cfg(feature = "rocksdb-backend")]
impl RocksDBEngine {
    pub fn open(path: &Path) -> Result<Self, KilnchainError> {
    ...
    fn cf_handle(&self, cf: &str) -> Result<&rocksdb::ColumnFamily, KilnchainError> {
        self.db
            .cf_handle(cf)
            .ok_or_else(|| KilnchainError::Storage(format!("column family '{}' not found", cf)))
    }
}
