<!-- AGENTS.md —— Kilnchain 项目 AI 编码代理指南 -->

## 项目概览

**Kilnchain** 是一个高性能区块链核心库，使用 **Rust** 实现底层密码学、共识原语、P2P 网络、EVM 执行与数据结构，并通过 **PyO3** 向 Python 提供符合 Python 生态习惯的绑定层。目标用户为数据科学家、量化研究员及需要快速原型验证的区块链开发者。
...
```
kilnchain/
├── Cargo.toml                 # Rust workspace 根配置（11 个 member）
...
├── src/
│   ├── kilnchain/            # Python 源码包（Layer 3 封装）
│   │   ├── __init__.py        # 统一导出公共 API + __all__
│   │   ├── types.py           # Pydantic v2 输入校验模型（TxInput / BlockInput）
│   │   ├── client.py          # 高层 Pythonic API（open_db 异步上下文管理器）
│   │   ├── vm.py              # EVM 执行层高层封装（EvmState / EvmExecutor / ExecutionResult）
│   │   └── py.typed           # PEP 561 类型标记
│   └── tests/
│       ├── conftest.py        # pytest 共享配置（当前为空）
│       ├── unit/              # 单元测试（异常、密码学、Merkle、存储、类型、Pydantic、客户端、EVM、Mempool、共识、P2P、轻客户端、区块）
│       └── integration/       # 集成测试（端到端链路测试）
├── crates/
│   ├── kilnchain-error/      # 统一错误类型 KilnchainError（thiserror）
│   ├── kilnchain-crypto/     # 密码学原语：SHA-256、Keccak-256、RIPEMD-160、secp256k1 ECDSA
│   ├── kilnchain-core/       # 核心结构：Transaction、BlockHeader、Block、MerkleTree、RLP 编解码、LightClient、MPT
│   ├── kilnchain-storage/    # 存储抽象 Trait + InMemoryStorage + CachedStorage（LRU）+ RocksDBEngine（可选 feature）
│   ├── kilnchain-py/         # PyO3 绑定层：暴露全部 Rust 核心类型到 Python
│   ├── kilnchain-p2p/        # P2P 网络层：Noise 握手、Message 编解码、Kademlia 路由表、Gossip 广播、Sync 同步、Node 集成
│   ├── kilnchain-mempool/    # 交易内存池：CRUD、优先级队列、nonce 验证、容量限制与驱逐
│   ├── kilnchain-block-producer/  # 区块生产者：BlockBuilder、从 mempool 取交易构建区块
│   ├── kilnchain-consensus/  # HotStuff BFT 共识：BlockTree、Vote、QC、SafetyRules、Pacemaker、ConsensusEngine
│   ├── kilnchain-evm/        # EVM 执行层：revm 集成、转账、合约部署、合约调用、DatabaseCommit
│   └── kilnchain-rpc/        # JSON-RPC 服务层：axum HTTP + WebSocket、eth_sendRawTransaction/eth_getBalance/eth_call/eth_subscribe 等
└── design/                    # 架构设计文档（中文撰写）
...
### 1. kilnchain-error
- **职责：** 整个 workspace 的统一错误枚举 `KilnchainError`
- **变体：** `Crypto(String)`、`Serialization(String)`、`Storage(String)`、`InvalidParameter(String)`、`StateRootMismatch { expected, actual }`
- **依赖：** 仅 `thiserror`
...
- **基准：** `benches/sign_bench.rs`（签名吞吐量）
...
- **方法：** eth_chainId、net_version、eth_sendRawTransaction、eth_getBalance、eth_getTransactionCount、eth_getBlockByNumber、eth_getBlockByHash、eth_call、eth_getCode、eth_blockNumber、eth_subscribe / eth_unsubscribe
- **测试：** 含异步 HTTP 测试（使用 axum + tower::ServiceExt）

### 11. kilnchain-py
- **职责：** 唯一依赖 PyO3 的 crate，将 Rust 核心类型暴露为 Python 扩展模块 `kilnchain._internal`
- **暴露类型：**
  - `PyMerkleTree`、`PyTransaction`、`PyBlockHeader`、`PyBlock`、`PyLightClient`、`PyMptProof`
  - `PySecretKey`、`PyPublicKey`
  - `PyInMemoryStorage`、`PyCachedStorage`、`PyRocksDBEngine`（可选 feature `rocksdb-backend`）
  - `PyMempool`、`PyBlockBuilder`
  - `PyEvmState`、`PyEvmExecutor`、`PyExecutionResult`
  - `PyPhase`、`PyVote`、`PyQuorumCertificate`、`PyBlockNode`、`PyBlockTree`、`PySafetyRules`、`PyLeaderRotator`、`PyPacemaker`、`PyConsensusEngine`
  - `PyPeerId`、`PyPeerInfo`、`PyMessage`、`PyNodeConfig`、`PyRoutingTable`、`PyNode`
- **错误映射：** `KilnchainError` → 细化的自定义异常：`KilnchainValueError`（InvalidParameter / Serialization）、`KilnchainCryptoError`（Crypto）、`KilnchainStorageError`（Storage）、`KilnchainStateError`（StateRootMismatch）、`KilnchainRuntimeError`（其他）
...
from kilnchain._internal import (
    Block, BlockBuilder, BlockHeader, BlockNode, BlockTree, CachedStorage,
    KilnchainCryptoError, KilnchainError, KilnchainRuntimeError,
    KilnchainStateError, KilnchainStorageError, KilnchainValueError,
    ConsensusEngine, EvmExecutor, EvmState, ExecutionResult, InMemoryStorage,
    LeaderRotator, LightClient, MerkleTree, Message, Mempool, MptProof,
    Node, NodeConfig, Pacemaker, PeerId, PeerInfo, Phase, PublicKey,
    QuorumCertificate, RoutingTable, SafetyRules, SecretKey, Transaction,
    Vote, keccak256, ripemd160, sha256,
)
...
__all__ = [
    "Block", "BlockBuilder", "BlockHeader", "BlockInput", "BlockNode",
    "BlockTree", "CachedStorage", "KilnchainCryptoError", "KilnchainError",
    "KilnchainRuntimeError", "KilnchainStateError", "KilnchainStorageError",
    "KilnchainValueError", "ConsensusEngine", "EvmExecutor", "EvmState",
    "ExecutionResult", "InMemoryStorage", "LeaderRotator", "LightClient",
    "Mempool", "MerkleTree", "Message", "MptProof", "Node", "NodeConfig",
    "Pacemaker", "PeerId", "PeerInfo", "Phase", "PublicKey",
    "QuorumCertificate", "RocksDBEngine", "RoutingTable", "SafetyRules",
    "SecretKey", "Transaction", "TxInput", "Vote", "keccak256", "open_db",
    "ripemd160", "sha256",
]
```
...
- `vm.py` —— EVM 执行层高层封装：`EvmState`（流式 setter 返回 self）、`EvmExecutor`、`ExecutionResult`（属性化访问 gas_used / output / contract_address）
...
| 类型检查 | `pixi run typecheck` | `mypy src/kilnchain` |
...
1. **rust-checks**（ubuntu-latest）
   ...
   - `cargo test --workspace --all-features --exclude kilnchain-py`
   - `cargo test -p kilnchain-py`
   - `cargo bench --workspace --no-run`

2. **python-checks**（矩阵：Ubuntu / macOS / Windows）
   - 安装 Pixi (`prefix-dev/setup-pixi@v0.9.6`)
   - `pixi run dev-build`
   - `pixi run test-py`
   - `pixi run typecheck`

3. **build-wheels**（依赖上述两个 job）
   - 运行于 Ubuntu、macOS-14、Windows
   - 使用 `PyO3/maturin-action@v1` 并启用 `sccache`
   - 上传 wheel 为 artifact

4. **publish-release**（仅在 tag push 时触发）
   - 下载全部 wheels
   - 使用 `softprops/action-gh-release@v2` 自动发布到 GitHub Releases
...
| 属性测试 | `proptest` | 随机交易序列、状态一致性、哈希输出长度 | `cargo test`（crypto、core、storage、mempool 等 crate） |
...
- 存储快照隔离
- Mempool 容量驱逐与优先级队列
- HotStuff 完整三阶段提交流水线（prepare → pre-commit → commit）
- EVM 转账、余额不足、合约部署与调用
- RPC HTTP 端点（chainId、sendRawTransaction、getBalance、call 等）
- P2P Node 消息去重、peer discovery、RoutingTable XOR 距离
...
| 异步测试 | `pytest-asyncio` | 异步存储读写、P2P Node 操作 |
| 类型检查 | `mypy` | Stub 文件与实际实现一致 |

**测试文件清单（共 97 个测试函数）：**
- `test_exceptions.py` —— KilnchainError 异常映射测试（5）
- `test_py_crypto.py` —— SecretKey 生成、签名验证、哈希函数（5）
- `test_py_merkle.py` —— MerkleTree root、proof、verify（2）
- `test_py_storage.py` —— InMemoryStorage / CachedStorage put/get/delete（4，含 async）
- `test_py_types.py` —— Transaction / BlockHeader 构造、hash、RLP 往返（5）
- `test_pydantic_types.py` —— TxInput / BlockInput 校验（有效值、默认值、负值、长度超限）（8）
- `test_py_block.py` —— Block / BlockBuilder / produce_block 构造、hash、RLP 往返（3）
- `test_py_mempool.py` —— Mempool 插入/查询/删除/优先级/驱逐/nonce 跟踪（5）
- `test_py_evm.py` —— EvmState / EvmExecutor 转账、部署、状态查询、ExecutionResult 属性（5）
- `test_py_light_client.py` —— LightClient 区块头同步、MptProof 构造（4）
- `test_consensus.py` —— HotStuff 共识：Phase / Vote / QC / BlockTree / SafetyRules / Pacemaker / LeaderRotator / ConsensusEngine（约 20）
- `test_p2p.py` —— P2P 网络：PeerId / PeerInfo / Message / NodeConfig / RoutingTable / Node（约 16，含 async）
- `test_client.py` —— `open_db` 上下文管理器（2，含 async）
- `test_e2e.py` —— 端到端签名验证、Merkle 树、BlockHeader hash（3）
...
- `KilnchainError` 通过 `into_py_err` 映射到细化的自定义 Python 异常：
...
- 代码注释与 docstring 使用 **中文**，以匹配现有设计文档风格
...
| `design/p2p/design.md` + `phases/p2p-01.md` | P2P 网络层设计（Noise、Kademlia、Gossip、Sync） |
| `design/consensus/design.md` + `phases/con-01.md` | HotStuff BFT 共识设计 |
| `design/evm/design.md` + `phases/evm-01.md` | EVM 兼容执行层设计 |
| `design/rpc/design.md` + `phases/rpc-01.md` ~ `rpc-04.md` | JSON-RPC API 层设计 |
| `design/mempool/design.md` | 交易池设计 |
| `design/block-producer/design.md` | 区块生产者设计 |
| `design/light-client/design.md` + `phases/lc-01.md` ~ `lc-04.md` | 轻客户端设计 |
| `design/python-api/design.md` | Python API 层设计 |
...
3. **新增 PyO3 暴露类型：** 仅在 `kilnchain-py` crate 中进行，并在 `src/kilnchain/__init__.py` 中重新导出。
4. **错误处理：** 新增 Rust 错误变体时，同步更新 `kilnchain-error/src/lib.rs` 与 `kilnchain-py/src/error.rs` 的映射逻辑。
